...
let DefaultSetup : Operation list = [Actions("Setup", fun (client, suite) ->
    ...
    let getAndDeleteFilter (setup:_ -> DynamicResponse) (delete:(_ -> DynamicResponse)) filter = 
        new Dictionary<string, DynamicValue>(setup().Dictionary)
        |> Seq.map (fun kv ->
            match filter with
            | Some filter ->
    ...
    let deleteAllTasks () = 
        ...
        let cancelJobs = 
            new Dictionary<string, DynamicValue>(getJobs.Get<DynamicDictionary> "nodes")
            |> Seq.collect(fun kv -> new Dictionary<string, DynamicValue>(kv.Value.Get<DynamicDictionary> "tasks"))
            |> Seq.map (fun kv ->
